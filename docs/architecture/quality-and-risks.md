...
| AR-11 | Medium | The OS-level Claude tool sandbox defaults to disabled for scan/verify. Fix mode now forces it on with fail-if-unavailable and no unsandboxed fallback, but Python/git orchestration remains outside that sandbox. | Enable `sandbox.enabled=true` and `fail_if_unavailable=true` in production baselines for every mode; isolate the entire Python/git process in an ephemeral container/VM with scoped network egress. |
