...
| QA-SEC-05 | Privileged remediation containment | A hostile report or target is processed in fix mode; it must not redirect delivery upstream, expand egress, escape report staging, or silently bypass gates | Host/path validation, bounded link-free staging, non-git scratch, private-fork runtime policy, locked tools/domains, recorded checkpoints, final schema/semantic checks | Ephemeral-worker adversarial test with prompt injection, symlinks, oversized reports, fake success dispositions, forbidden hosts, and upstream mutation canaries |
| QA-SEC-06 | Mythos containment | A classifier-free model processes a hostile checkout; it must have no host mount, GitHub credential, target settings, Bash, or egress beyond the exact inference endpoint | Exact model policy, retention gate, user-only settings, gVisor, internal network, CONNECT allow-list proxy, startup attestations | CI canaries for runtime fallback, direct IP egress, denied CONNECT, token/env leakage, project hook loading, and attempted host/Docker-socket access |
...
| Remediation → target repository | Tampering, elevation, repudiation | Worktrees, RED/GREEN evidence, scope/delivery gates, interactive approvals or recorded automated checkpoints, private-fork-only agent policy, credential-free/no-default-GitHub-egress dry run, final disposition checks | Delivery-enabled fix-mode Bash and GitHub token are intentionally privileged; target tests execute untrusted code and model adherence still matters |
...
- A contained automated remediation profile with bounded report intake, zero-mutation
  dry run, explicit human-required outcomes, and schema plus semantic result validation.
...
|---|---|---|---|
| AR-01 | Medium | The shared root `install.sh` now installs all three skills and creates the remediation venv, but the documented `uninstall.sh` is absent and there is no clean-home discovery smoke test. Installation is improved but the complete lifecycle is not reproducibly tested. | Add the matching uninstaller and a clean-home test that installs, discovers, invokes preflight for all three skills, and removes them. |
...
| AR-04 | High | The legacy root `bin/vuln-scan-harness` path remains incomplete: it sources `_sandbox_env.sh` and exits, `setup_sandbox.sh` references a missing legacy proxy Dockerfile/root editable package, and its setup path is not the GitHub Actions agent path. Mythos is explicitly rejected by `harness_core` to prevent this becoming a weaker bypass. | Retire the legacy launcher or restore it behind the same tested container policy. Until then, use `vulnhunter-agent` and `scripts/run_mythos_sandbox.sh`; add shell syntax and end-to-end behavioral tests. |
| AR-05 | Low | The known issue-closure and six-versus-seven-gate prose drift is corrected, but the interactive and automated checkpoint rules now span the root skill plus phase prompts without a dedicated sync lint. A later prompt edit could reintroduce conflicting control flow. | Add contract tests for `VULNFIX_AUTOMATED`, phase checkpoint behavior, dry-run prohibitions, and the seven-gate registry; generate summary inventories where practical. |
...
| AR-11 | Medium | The OS-level Claude tool sandbox defaults to disabled for ordinary Opus scan/verify. Fix and Mythos force it on with fail-if-unavailable and no unsandboxed fallback, but ordinary Python/git orchestration remains outside that sandbox. | Enable `sandbox.enabled=true` and `fail_if_unavailable=true` in every production baseline; use the Mythos two-plane gVisor profile as the reference containment pattern. |
...
| AR-18 | High | A delivery-enabled automated fix must expose a target/fork-capable GitHub token to a Bash-enabled model session. The token is absent from argv/files/remotes and egress is scoped, but a compromised model/tool invocation can still read its environment and misuse the token against allowed GitHub endpoints. `--no-post` avoids this exposure by pre-cloning, then blanking model GitHub credentials and default GitHub egress. | Use a short-lived fine-grained/App token restricted to the intended repositories and permissions; isolate one run per worker; add a broker/proxy that mediates allowed GitHub operations rather than exposing the raw token where feasible. |
| AR-19 | High | Fix mode executes repository tests and dependency tooling from an untrusted target. The Claude tool sandbox limits paths/network but does not make hostile test code safe, and operators may add package-registry egress. | Run fix jobs in disposable VMs/containers with no unrelated mounts/secrets, strict CPU/memory/time limits, read-only host filesystem, package allow-lists, and no cloud metadata access. Treat sandbox disablement as a deployment exception requiring compensating isolation. |
| AR-20 | Medium | A delivery-enabled fix session resolves a brokered scan token once and keeps it in the Claude process environment for a potentially long remediation run. Rotation or expiry mid-run can cause partial delivery, and the model cannot safely refresh broker state itself. | Mint a token lifetime exceeding the bounded job duration or mediate GitHub through a refresh-capable sidecar/wrapper; record token expiry metadata and fail before starting when remaining lifetime is insufficient. |
| AR-21 | Medium | `fix_disposition.json` is schema- and semantically validated, but its evidence and reported GitHub URLs are still authored by the remediation session. The Python layer does not independently reconcile commits, branches, issues, PRs, and gate outputs with GitHub/local state. | Add a post-session reconciliation pass that reads deterministic result/gate artifacts, inspects local git, and queries GitHub read-only before accepting `COMPLETED`; generate the disposition in Python from reconciled facts rather than trusting model aggregation. |
| AR-22 | High | Mythos has no safety classifiers and provider requests are retained for 30 days without ZDR. Container isolation limits execution authority but cannot undo disclosure of source included in inference prompts. | Restrict Mythos to repositories whose data classification permits provider retention; use a dedicated workspace/key, protected environment approval, explicit per-run acknowledgement, and provider-side access/audit review. |
| AR-23 | Medium | The Mythos proxy enforces a DNS name rather than fixed AWS IPs, and the image build occurs in the more privileged control plane with general egress. A compromised base/package supply chain remains outside gVisor's runtime boundary. | Build, scan, sign, and digest-pin images in a separate release pipeline; pull from a private registry; monitor proxy destination/SNI decisions; keep TLS validation enabled. |

## 7. Recommended improvement order

1. **Constrain privileged fix execution:** require ephemeral workers, scoped short-lived
   tokens, strict resource/egress policy, and independent post-session reconciliation.
2. **Finish deployability:** add the shared uninstaller and clean-home discovery test;
   restore the real harness launch/setup path and resolve the empty compatibility script.
3. **Enforce verifier write isolation:** make evidence roots mechanically read-only and
   the output directory the only writable model path.
4. **Reconcile and test contracts:** totalize verifier gate mapping; single-source exit
   codes, gate inventory, issue-closure behavior, and schema versions; add skill fixtures.
5. **Harden production isolation:** fresh clones, sandbox-on baseline, ephemeral workers,
   egress policy, short-lived identities, and telemetry governance.
6. **Remove probabilistic structure extraction:** produce findings JSON directly from
   the audit and reconcile it with report evidence.
7. **Improve concurrent operation:** publication concurrency strategy and per-request
   broker token refresh throughout verify.
8. **Operationalize quality:** benchmark precision/recall, graph fallback rate, scan
