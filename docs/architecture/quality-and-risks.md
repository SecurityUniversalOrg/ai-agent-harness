...
| Checkout → model tools | Elevation, information disclosure, denial of service | Default Bash removal, tool visibility restriction, optional sandbox, source scope instructions, task-stall limit | Sandbox is disabled by default; executable scans intentionally run untrusted project tooling after opt-in |
...
| GitHub issue/comment → verify | Tampering, spoofing, injection, denial of service | Edit reconstruction, strict marker grammar, untrusted boundaries, pagination/byte caps, URL allow-list and aliases | Verification skill is absent from this snapshot, so enforcement on the model side cannot be audited here |
...
|---|---|---|---|
| AR-01 | High | The agent invokes `/vulnhunt-fix-verify`, and both subsystem READMEs require root `install.sh`/`uninstall.sh`, but the verification skill and installers are absent from this snapshot. Scan/verify deployment cannot be reproduced from this checkout alone. | Restore or vendor the missing skill and installers, or declare/version them as external release artifacts. Add a packaging smoke test that installs all skills into a clean home and launches both slash commands. |
| AR-02 | High | `git status` shows `bin/`, `scripts/`, `vulnhunt/`, `vulnhunter-agent/`, and `vulnhunter-fix/` as untracked. CI, review, provenance, and releases may omit the analyzed implementation. | Commit the intended source or explicitly exclude generated/vendor content. Require a clean-tree/source-manifest check in build and release workflows. |
...
| AR-04 | High | `scripts/verify_sandbox_env.sh` contains Python project TOML rather than a shell verification script, while `bin/vuln-scan-harness` contains only strict-shell setup and performs no launch. Packaging health checks can silently do nothing or fail unexpectedly. | Restore the intended files and add shell syntax plus behavioral smoke tests (`bash -n`, expected usage/exit/output). |
...
| AR-12 | Medium | Native Windows support is not established: remediation depends on Bash, paths and sandbox policy are POSIX-oriented, and skill discovery assumes Unix home locations in some paths. | Declare the supported deployment matrix. Test Linux container and macOS interactive paths; use WSL/container guidance or implement/test native Windows adapters before claiming support. |
...
1. **Restore provenance and deployability:** resolve untracked source, missing installers,
   missing verification skill, no-op launcher, and malformed sandbox script.
2. **Reconcile contracts:** make exit codes, gate inventory, issue-closure behavior, and
   schema versions single-sourced and contract-tested.
3. **Harden production isolation:** fresh clones, sandbox-on baseline, ephemeral workers,
   egress policy, short-lived identities, and telemetry governance.
4. **Remove probabilistic structure extraction:** produce findings JSON directly from
   the audit and reconcile it with report evidence.
5. **Improve concurrent operation:** publication concurrency strategy and per-request
   broker token refresh throughout verify.
6. **Operationalize quality:** benchmark precision/recall, graph fallback rate, scan
